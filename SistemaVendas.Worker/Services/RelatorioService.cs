...
public class RelatorioService
{
    ...
    public async Task AtualizarStatusAsync(Guid id,string status)
    {
        ...
        if (relatorio == null)
        {
            Console.WriteLine($"Relatório {id} não encontrado");
            return;
        }
        ...
        if (status == "Concluído")
        {
            relatorio.DataConclusao = DateTime.Now;
        }

        await _context.SaveChangesAsync();

        Console.WriteLine($"Relatório {id} => {status}");

    }

    public async Task FinalizarAsync(Guid id,string caminhoArquivo)
    {
        ...
        relatorio.ArquivoGerado = caminhoArquivo;
